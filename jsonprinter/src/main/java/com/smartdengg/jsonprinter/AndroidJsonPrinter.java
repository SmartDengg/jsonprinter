...
class AndroidJsonPrinter implements Printer {
  ...
  private static final String EMPTY_JSON = HORIZONTAL_DOUBLE_LINE + " " + "Empty/Null json content";
  ...
  /** 将字符串转成Json格式进行输出 */
  private void printer(int priority, String tag, String json) {

    if (!compare(tag, JsonPrinter.TAG)) sLocalTag.set(tag);

    if (TextUtils.isEmpty(json)) {
      boxing(priority, tag, EMPTY_JSON);
      return;
    }
    try {
      if (json.startsWith("{")) {
        JSONObject jsonObject = new JSONObject(json);
        boxing(priority, tag, jsonObject.toString(JSON_INDENT));
        return;
      }
      if (json.startsWith("[")) {
        JSONArray jsonArray = new JSONArray(json);
        boxing(priority, tag, jsonArray.toString(JSON_INDENT));
        return;
      }
      innerError(tag, new IllegalArgumentException("Invalid Json"), json);
    } catch (JSONException e) {
      //innerError(tag, e.getCause().getMessage() + SEPARATOR + json);
  ...
  private void innerError(String tag, Throwable throwable, String message, Object... args) {
    ...
    String errorMessage = "";
    String[] parts = formatString(message, args).split(SEPARATOR);
    for (String part : parts) errorMessage += HORIZONTAL_DOUBLE_LINE + " " + part + SEPARATOR;
    boxing(Log.ERROR, tag, errorMessage);
  }

  private void boxing(int priority, String tag, String message) {
   ...
   /* byte[] bytes = message.getBytes();
    ...
    if (length <= LOGGER_ENTRY_MAX_PAYLOAD) {
      logContent(priority, tag, message);
    } else {
      for (int i = 0; i < length; i += LOGGER_ENTRY_MAX_PAYLOAD) {
        int count = Math.min(length - i, LOGGER_ENTRY_MAX_PAYLOAD);
  ...
  private void logHeaderContent(int priority, String tag, int methodCount) {
    ...
    log(priority, tag,
        HORIZONTAL_DOUBLE_LINE + " Thread: " + Thread.currentThread().toString() + SEPARATOR);
  ...
  private void logContent(int priority, String tag, String content) {
    log(priority, tag, content);
  ...
  @Override public void v(String tag, String message) {
    this.printer(Log.VERBOSE, tag, message);
  }

  @Override public void i(String tag, String message) {
    this.printer(Log.INFO, tag, message);
  }

  @Override public void d(String tag, String message) {
    this.printer(Log.DEBUG, tag, message);
  }

  @Override public void w(String tag, String message) {
    this.printer(Log.WARN, tag, message);
  }

  @Override public void e(String tag, String message) {
    this.printer(Log.ERROR, tag, message);
  }

  @Override public void wtf(String tag, String message) {
    this.printer(Log.ASSERT, tag, message);
  }

  @Override public synchronized void log(int priority, String tag, String message) {
    if (message.length() < LOGGER_ENTRY_MAX_PAYLOAD) {
      if (priority == Log.ASSERT) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.FROYO) Log.wtf(tag, message);
      } else {
    ...
    for (int i = 0, length = message.length(); i < length; i++) {
      ...
      do {
        ...
        //noinspection StringBufferReplaceableByString
        StringBuilder part = new StringBuilder(3).append(HORIZONTAL_DOUBLE_LINE)
            .append(' ')
            .append(message.substring(i, end));

        if (priority == Log.ASSERT) {
          if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.FROYO) Log.wtf(tag, part.toString());
        } else {
          Log.println(priority, tag, part.toString());
        }
        i = end;
      } while (i < newline);
    }
